...
# http://chatbot-rasa-module-name:5005, call ok
URL_RASA = os.environ['URL_CHATBOT_RASA']
...
def response():
    ...
    if response["intent"]["name"] == 'predict_disease':
        ...
        dict['anwser'] = get_disease_info(response)[0]

    elif response["intent"]["name"] == 'ask_covid_total_case':
        dict['intent'] = "covid_total"
        call = requests.get(URL_COVID + '/covid-overview').json()
        json_data = call['data']['internal']
        today = datetime.datetime.now()
        dict['anwser'] =  \
            "Tình hình covid-19 trong nước ngày {}/{}:\nSố ca nhiễm: {}\nSố ca tử vong: {}\nSố ca phục hồi: {}\n".format(
                today.day, today.month, json_data['casesToday'], json_data['death'], json_data['recovered'])

    elif response["intent"]["name"] == 'ask_covid_domestic_case':
        dict['intent'] = "covid_domestic"
        json_data = get_domestic_info(response)
        today = datetime.datetime.now()
        dict['anwser'] =  \
            "Tình hình covid-19 tỉnh/thành phố {} ngày {}/{}:\nSố ca nhiễm: {}\nSố ca tử vong: {}\nSố ca phục hồi: {}\n".format(
                json_data['name'], today.day, today.month,  json_data['casesToday'], json_data['death'], json_data['recovered'])
